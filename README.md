...
New AI generation uses HTTP APIs without an agent CLI, with **Muse Spark 1.3 Contributor** as the default (`META_API_KEY`). Select **DeepSeek V4.1 Flash** using `--provider deepseek` (model ID `deepseek-flash`) through the existing local gateway at `http://127.0.0.1:35001/v1`. Both task drivers support this flag. No Codex CLI or its system prompt is involved. Model settings live in [`scripts/llm_models.toml`](scripts/llm_models.toml); see [configuration and protocol](docs/llm_protocol.md) and the [September 24 live pilot results](docs/reports/direct_api_pilots.md). Both models generated independently verified skid8 certificates and RTL rewrites. DeepSeek needed a 65,536-token output cap for the rewrite; the earlier official-API 402 and gateway 403 are historical failures of different routes or requests. DeepSeek upstream credentials belong to the gateway. Historical Codex results above retain their original labels and measurements.
